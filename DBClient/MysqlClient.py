...
class MysqlClient(object):
    ...
    def getAppkey_app(self, plat = ["android", "ios", "all", "feeling"]):
    ...
    def table_drop_table(self, db_name, table_name):
        sql = "drop table %(db_name)s.%(table_name)s" % {
            "db_name": db_name,
            "table_name": table_name,
        }
        print sql
...
if __name__ == "__main__":
    ...
    print(json.dumps(tester.getAppkey_app(plat=plats)))
    ...
    for data in tester.getAppkey_kwargs(plat=plats):
        ...
        if appkey == "caiyu_ad":
            continue
        table_name = "%(appkey)s_%(plat)s_user_conf" % {"appkey": appkey, "plat": plat}
        tester.table_drop_table(db_name, table_name)
