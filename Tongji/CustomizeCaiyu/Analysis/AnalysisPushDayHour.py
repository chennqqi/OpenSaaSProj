...
class AnalysisPushDayHour(AnalysisMap):

    def __init__(self):
        super(AnalysisPushDayHour, self).__init__()
        self.titles = [
                        u"P2P对账单",
                        u"自动对账单已出",
                        u"每月账单日",
    ...
    def rules(self, analysisresult, data, num, *args, **kwargs):
        ...
        # 推送
            ...
            result.setdefault("push_user", {}).setdefault(time_range, {}).setdefault(uid, {}).setdefault(title, set()).add(opts)
        else:
            result.setdefault("all_user", {}).setdefault(uid, set()).add(opts)
    ...
    def transformresult(self, analysisresult, *args, **kwargs):
        ...
        for _time_range in push_user:
            for uid in push_user[_time_range]:
                for title in push_user[_time_range][uid]:
                    result.setdefault((title, _time_range), [set(), set(), set(), set()])
                    # 推送打开人数
                    result[(title, _time_range)][0].add(uid)
                    push_ts = min(push_user[_time_range][uid][title])
