...
from ObfuscationCode import GetObfuscation
import StringIO
...
def format_snake(fn, raw=False, compress=False, binary=False):
  ...
  # fd = StringIO.StringIO(GetObfuscation(fn))
  fd = StringIO.StringIO(GetObfuscation(fn, "pyob.oxyry.com"))
...
if __name__ == '__main__':
  ...
  confuseCode.append(BREEDER_PREAMBLE % header.strip())
  if gtk_images:
    print BREEDER_GTK_PREAMBLE
    confuseCode.append(BREEDER_GTK_PREAMBLE)
  # print "sasdasd"*500, args

  for fn in args:
    # print BREEDER_DIVIDER
    # print breed(fn, (fn == args[-1]), gtk_images=gtk_images, compress=compress)
    # print
    confuseCode.append(BREEDER_DIVIDER)
    confuseCode.append(breed(fn, (fn == args[-1]), gtk_images=gtk_images, compress=compress))
    confuseCode.append("")
  # print BREEDER_POSTAMBLE
  confuseCode.append(BREEDER_POSTAMBLE)
  from os import sys, path
  base_path = path.dirname(__file__)
  filename = "decipherTransform_pack.py"
  out = open("/".join([base_path, filename]), "w")
  code = "\n".join(confuseCode)
  out.write(code)
  out.close()

  reader = open("/".join([base_path, filename]), "r")
  text = reader.read()
  reader.close()
  code = GetObfuscation("/".join([base_path, filename]))
  writer = open("/".join([base_path, filename]), "w")
  writer.write(code)
  writer.close()
